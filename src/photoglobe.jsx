...
function GlobeCanvas({photos,selectedId,onSelect,rotation,onRotate,globeScale,onScaleChange}){
  ...
  const draw=useCallback(()=>{
    ...
    for(let fi=0;fi<feats.length;fi++){const f=feats[fi];const polys=f.type==="MultiPolygon"?f.coords:[f.coords];for(const poly of polys)for(const ring of poly){const segs=clipRing(ring,rotation.lat,rotation.lon,R,cx,cy);for(const seg of segs){let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity;for(const p of seg){if(p.x<minX)minX=p.x;if(p.x>maxX)maxX=p.x;if(p.y<minY)minY=p.y;if(p.y>maxY)maxY=p.y;}if(maxX-minX>R*1.3||maxY-minY>R*1.3)continue;ctx.beginPath();ctx.moveTo(seg[0].x+3,seg[0].y+4);for(let i=1;i<seg.length;i++)ctx.lineTo(seg[i].x+3,seg[i].y+4);ctx.fillStyle="rgba(0,20,40,0.3)";ctx.fill();}}}
    for(let fi=0;fi<feats.length;fi++){const f=feats[fi];const colors=featColors[fi];const polys=f.type==="MultiPolygon"?f.coords:[f.coords];for(const poly of polys)for(const ring of poly){const segs=clipRing(ring,rotation.lat,rotation.lon,R,cx,cy);for(const seg of segs){let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity;for(const p of seg){if(p.x<minX)minX=p.x;if(p.x>maxX)maxX=p.x;if(p.y<minY)minY=p.y;if(p.y>maxY)maxY=p.y;}if(maxX-minX>R*1.3||maxY-minY>R*1.3)continue;ctx.beginPath();ctx.moveTo(seg[0].x,seg[0].y);for(let i=1;i<seg.length;i++)ctx.lineTo(seg[i].x,seg[i].y);ctx.fillStyle=colors.fill;ctx.fill();ctx.strokeStyle=colors.stroke;ctx.lineWidth=0.6;ctx.stroke();}}}
